...
var ScKpiBase = /** @class */ (function () {
    function ScKpiBase(hostRef) {
        ...
        this.getValues = function () {
            if (!_this.trendStream || !_this.trendStream.data.length) {
                return {
                    latestPoint: undefined,
                    previousPoint: undefined,
                };
            }
            var latestPoint = _this.trendStream.data[_this.trendStream.data.length - 1];
            var previousPoint = _this.trendStream.data[_this.trendStream.data.length - 2];
            return {
        ...
        this.updateName = function (name) {
            if (_this.propertyStream) {
                _this.onChangeLabel({
                    streamId: _this.propertyStream.id,
                    name: name,
                });
            }
            else if (_this.alarmStream) {
                _this.onChangeLabel({
                    streamId: _this.alarmStream.id,
                    name: name,
                });
            }
        };
        this.fontColor = function (latestPoint) {
            if (!_this.isEnabled) {
            ...
            return _this.valueColor || DEFAULT_FONT_COLOR;
        };
        this.fontSize = function () { return (_this.miniVersion ? MINI_FONT_SIZE : FONT_SIZE); };
        this.iconSize = function () { return _this.fontSize() * ICON_SHRINK_FACTOR; };
    }
    ScKpiBase.prototype.render = function () {
        var _a = this.getValues(), latestPoint = _a.latestPoint, previousPoint = _a.previousPoint;
        var shouldShowTrends = this.isEnabled &&
            previousPoint &&
            latestPoint &&
            this.trendStream &&
            this.trendStream.dataType !== DataType.STRING;
        var stream = this.propertyStream || this.alarmStream;
        var point = this.propertyStream ? this.propertyPoint : this.alarmPoint;
        var icon = this.breachedThreshold ? this.breachedThreshold.icon : undefined;
        if (stream == null) {
            return undefined;
        }
        var error = this.propertyStream && this.propertyStream.error;
