...
import { b as bisector } from './utils-96fe4147.js';
 ...
 */
var getVisibleData = function (data, viewport, 
...
includeBoundaryPoints) {
    ...
    var start = isMinimalStaticViewport(viewport)
        ? new Date(viewport.start)
        : new Date(Date.now() - parseDuration(viewport.duration));
    var end = isMinimalStaticViewport(viewport) ? new Date(viewport.end) : new Date();
    // If there is no data
    if (data.length === 0) {
        return [];
    }
 ...
 */
var getDataBeforeDate = function (data, date) {
    // If there is no data
    if (data.length === 0) {
        return [];
    }
    ...
    // Otherwise return all the data within the viewport, plus an additional single data point that falls outside of
    // the viewport in either direction.
    var endIndex = Math.min(pointBisector.right(data, date) - 1, data.length - 1);
