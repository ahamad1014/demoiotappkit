...
const pointBisector = utils.bisector((p) => p.x);
/**
 * Get Visible Data
 *
...
includeBoundaryPoints = true) => {
    const start = predicates.isMinimalStaticViewport(viewport)
        ? new Date(viewport.start)
        : new Date(Date.now() - time.parseDuration(viewport.duration));
    const end = predicates.isMinimalStaticViewport(viewport) ? new Date(viewport.end) : new Date();
    // If there is no data
    if (data.length === 0) {
        return [];
    }
