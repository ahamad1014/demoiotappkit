...
const scStatusTimelineCss = "sc-status-timeline .status-timeline{position:relative}sc-threahold-legend{overflow-y:scroll;overflow-x:hidden;-ms-overflow-style:none;scrollbar-width:none}sc-threshold-legend::-webkit-scrollbar{display:none}";
...
const DEFAULT_MARGINS = {
    marginLeft: 10,
    marginTop: 0,
    marginBottom: chartDefaults.DEFAULT_CHART_CONFIG.size.marginBottom,
    marginRight: 5,
};
...
const tooltip = (alarms) => (props) => {
    const { size } = props;
    return (index.h("sc-tooltip", Object.assign({}, props, { dataAlignment: constants.DATA_ALIGNMENT.LEFT, top: -size.height + chartScene.STATUS_MARGIN_TOP_PX + TOP_TOOLTIP_MARGIN_PX, sortPoints: false, maxDurationFromDate: alarms ? alarms.expires : undefined, showDataStreamColor: false, showBlankTooltipRows: true, visualizesAlarms: true, supportString: true })));
};
const getComponentViewport = (viewport) => (Object.assign(Object.assign({}, viewport), { yMin: 0, yMax: chartScene.HEIGHT }));
const ScStatusTimeline = class {
    constructor(hostRef) {
        ...
        this.thresholds = () => {
            ...
            return this.annotations.y.filter(utils.isThreshold);
        };
    }
    componentWillRender() {
    ...
    render() {
        return (index.h("sc-size-provider", { size: this.size, renderFunc: (size) => {
                const totalSize = Object.assign(Object.assign(Object.assign(Object.assign({}, chartDefaults.DEFAULT_CHART_CONFIG.size), DEFAULT_MARGINS), this.size), size);
                ...
                return [
                    index.h("div", { class: "status-timeline", style: { height: `${chartSize.height}px` } }, index.h("sc-webgl-base-chart", { axis: Object.assign(Object.assign({}, this.axis), { showY: false }), gestures: this.gestures, configId: this.widgetId, annotations: Object.assign(Object.assign({}, this.annotations), { show: false, thresholdOptions: {
                                showColor: true,
                            } }), supportedDataTypes: [dataConstants.DataType.NUMBER, dataConstants.DataType.STRING, dataConstants.DataType.BOOLEAN], updateChartScene: chartScene.updateChartScene, createChartScene: chartScene.chartScene, size: chartSize, dataStreams: this.dataStreams, alarms: this.alarms, viewport: this.componentViewport, minBufferSize: this.minBufferSize, bufferFactor: this.bufferFactor, isEditing: this.isEditing, renderTooltip: tooltip(this.alarms), displaysError: false, supportString: true, visualizesAlarms: true, displaysNoDataPresentMsg: false, messageOverrides: this.messageOverrides }), index.h("sc-status-timeline-overlay", { isEditing: this.isEditing, thresholds: this.thresholds(), date: predicates.isMinimalStaticViewport(this.viewport) ? new Date(this.viewport.end) : new Date(), dataStreams: this.dataStreams, size: chartSize, widgetId: this.widgetId })),
                    index.h("div", { class: "threshold-legend-container", style: { maxHeight: `${THRESHOLD_LEGEND_HEIGHT_PX}px` } }, index.h("sc-threshold-legend", { thresholds: this.thresholds() })),
                ];
            } }));
    }
...
const scStatusTimelineOverlayCss = "sc-status-timeline-overlay .expando{-ms-flex-positive:1;flex-grow:1}sc-status-timeline-overlay .overlay-container{position:absolute;display:-ms-flexbox;display:flex;-ms-flex-direction:column;flex-direction:column;-ms-flex-pack:justify;justify-content:space-between}";
...
const ScStatusTimelineOverlay = class {
    ...
    render() {
        ...
        return (index.h("div", { class: "overlay-container", style: {
            ...
            const point = breachedThreshold.closestPoint(getDataPoints.getDataPoints(dataStream, dataStream.resolution), this.date, constants.DATA_ALIGNMENT.LEFT);
            ...
            return (index.h("sc-status-timeline-overlay-row", { key: dataStream.id, label: dataStream.name, detailedLabel: dataStream.detailedName, value: displayedValue, unit: displayedUnit, isEditing: this.isEditing, valueColor: valueColor, icon: error == null ? threshold && threshold.icon : constants.StatusIcon.ERROR, onNameChange: (name) => this.onChangeLabel({ streamId: dataStream.id, name }) }));
        })));
    }
};
ScStatusTimelineOverlay.style = scStatusTimelineOverlayCss;

const scStatusTimelineOverlayRowCss = ":root{--timeline-row-margin-top:34px}sc-status-timeline-overlay-row{-ms-flex-positive:1;flex-grow:1;width:100%;display:-ms-flexbox;display:flex;-ms-flex-direction:column;flex-direction:column;-ms-flex-pack:justify;justify-content:space-between}sc-status-timeline-overlay-row sc-data-stream-name{max-height:var(--timeline-row-margin-top);overflow:hidden}sc-status-timeline-overlay-row .value{max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding-bottom:2px}sc-status-timeline-overlay-row .unit{padding-left:2px}sc-status-timeline-overlay-row .stream-info{text-overflow:ellipsis;overflow:hidden;height:var(--timeline-row-margin-top);display:-ms-flexbox;display:flex;font-size:14px;padding:0.1em;-ms-flex-align:end;align-items:flex-end}sc-status-timeline-overlay-row .no-data-visualization{-ms-flex-positive:1;flex-grow:1;background:repeating-linear-gradient(45deg, #efefef, #efefef 10px, #acacac 10px, #acacac 20px)}";
...
const ScStatusTimelineOverlayRow = class {
    ...
    render() {
        return [
            index.h("div", { class: "stream-info" }, index.h("sc-data-stream-name", { label: this.label, detailedLabel: this.detailedLabel, onNameChange: this.onNameChange, isEditing: this.isEditing }), index.h("div", { class: "expando" }), index.h("span", { class: "value", style: { color: this.valueColor || 'unset', display: 'flex', alignItems: 'center' } }, this.icon && index.h("sc-chart-icon", { name: this.icon }), index.h(Value.Value, { value: this.value, unit: this.unit }))),
            index.h("div", { class: "no-data-visualization" }),
        ];
    }
};
ScStatusTimelineOverlayRow.style = scStatusTimelineOverlayRowCss;

var lodash_uniqby = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/**
...
const scThresholdLegendCss = "sc-threshold-legend{display:-ms-flexbox;display:flex;-ms-flex-wrap:wrap;flex-wrap:wrap;overflow-y:scroll;font-size:14px;line-height:22px}sc-threshold-legend-row{padding:0.1em 1em}sc-threshold-legend-row .box{display:inline-block;border-radius:2px;margin-right:8px;--box-size:12px;width:var(--box-size);height:var(--box-size)}";

const COMPARISON_SYMBOL = {
    [constants.COMPARISON_OPERATOR.EQUAL]: '=',
    [constants.COMPARISON_OPERATOR.LESS_THAN]: '<',
    [constants.COMPARISON_OPERATOR.LESS_THAN_EQUAL]: '<=',
    [constants.COMPARISON_OPERATOR.GREATER_THAN]: '>',
    [constants.COMPARISON_OPERATOR.GREATER_THAN_EQUAL]: '>=',
};
const label = (threshold) => {
    if (threshold.comparisonOperator === constants.COMPARISON_OPERATOR.EQUAL) {
...
const ScThresholdLegend = class {
    ...
    render() {
        return this.uniqueThresholds().map(threshold => (index.h("sc-threshold-legend-row", { key: key(threshold), color: threshold.color, label: label(threshold) })));
    }
};
...
const ScThresholdLegendRow = class {
    ...
    render() {
        return (index.h("div", null, index.h("div", { class: "box", style: { backgroundColor: this.color } }), " ", this.label));
    }
};

exports.sc_status_timeline = ScStatusTimeline;
exports.sc_status_timeline_overlay = ScStatusTimelineOverlay;
exports.sc_status_timeline_overlay_row = ScStatusTimelineOverlayRow;
exports.sc_threshold_legend = ScThresholdLegend;
exports.sc_threshold_legend_row = ScThresholdLegendRow;
