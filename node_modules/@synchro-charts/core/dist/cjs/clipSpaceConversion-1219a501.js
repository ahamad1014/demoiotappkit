...
const getCSSColorByString = (color) => {
    const cssColor = index$2.colorString.get(color);
    if (cssColor == null) {
...
const vertices = (stream, resolution) => {
    ...
    return getDataPoints.getDataPoints(stream, resolution).map(p => [p.x, p.y, r, g, b]);
};
...
const constructChartScene = ({ scene, container, viewport, toClipSpace, onUpdate, }) => {
    ...
    const camera = new three_module.OrthographicCamera(toClipSpace(viewport.start.getTime()), toClipSpace(viewport.end.getTime()), viewport.yMax, viewport.yMin, NEAR, FAR);
 ...
 */
const numDataPoints = (dataStreams) => dataStreams.map(stream => getDataPoints.getDataPoints(stream, stream.resolution).length).reduce((total, num) => total + num, 0);
...
const granularity = (durationMS) => {
    if (durationMS < 10 * time.MINUTE_IN_MS) {
        // single millisecond
        return 1;
    }
    if (durationMS < time.DAY_IN_MS) {
        return time.SECOND_IN_MS / 10;
    }
    if (durationMS < time.DAY_IN_MS * 7) {
        return time.SECOND_IN_MS;
    }
    if (durationMS < time.YEAR_IN_MS) {
        return time.MINUTE_IN_MS;
    }
    if (durationMS < 30 * time.YEAR_IN_MS) {
        return 30 * time.MINUTE_IN_MS;
    }
...
exports.clipSpaceConversion = clipSpaceConversion;
exports.constructChartScene = constructChartScene;
exports.getCSSColorByString = getCSSColorByString;
exports.needsNewClipSpace = needsNewClipSpace;
exports.numDataPoints = numDataPoints;
