...
const Arrow = () => (index.h("svg", { viewBox: "0 0 24 24" },
    index.h("path", { d: "M13.025 1l-2.847 2.828 6.176 6.176h-16.354v3.992h16.354l-6.176 6.176 2.847 2.828 10.975-11z" })));
...
const Trend = ({ previousPoint: { y: prevY }, latestPoint: { y: latestY }, miniVersion, }) => {
    ...
    return (index.h("div", { class: classes },
        index.h("div", { class: "data" },
            index.h("div", { class: "direction" },
                index.h(Arrow, null)),
            index.h("div", { "data-testid": "previous-value", class: "value" }, computePercentage(prevY, latestY))),
        !miniVersion && index.h("div", { class: "trend-description" }, "from previous value")));
};
...
const ScKpiBase = class {
    ...
    render() {
        ...
        const shouldShowTrends = this.isEnabled &&
            ...
            latestPoint &&
            this.trendStream &&
            this.trendStream.dataType !== dataConstants.DataType.STRING;
        ...
        return (index.h("div", { class: { wrapper: true, large: !this.miniVersion } }, index.h("div", null), index.h("sc-data-stream-name", { displayTooltip: false, label: stream.name, detailedLabel: stream.detailedName, pointType: "data" /* DATA */, date: point && new Date(point.x), onNameChange: this.updateName, isEditing: this.isEditing }), index.h("div", { class: "icon-container" }, this.isEnabled && icon && index.h("sc-chart-icon", { name: icon, size: this.iconSize(), color: this.valueColor })), index.h("div", { class: { main: true, large: !this.miniVersion } }, error != null && index.h("sc-error-badge", { "data-testid": "warning" }, error), this.isLoading ? (index.h("sc-loading-spinner", { "data-testid": "loading", style: { height: `${this.fontSize()}px`, width: `${this.fontSize()}px` } })) : (index.h("div", { "data-testid": "current-value", class: { 'value-wrapper': true, large: !this.miniVersion }, style: { color: this.fontColor(point) } }, index.h(Value.Value, { isEnabled: this.isEnabled, value: point ? point.y : undefined, unit: stream.unit }))), shouldShowTrends && this.isEnabled && (index.h(Trend, { previousPoint: previousPoint, latestPoint: latestPoint, miniVersion: this.miniVersion })), !shouldShowTrends && this.isEnabled && point && (index.h("div", null, "at", ' ', new Date(point.x).toLocaleString('en-US', {
            hour: 'numeric',
            minute: 'numeric',
        }))))));
    }
