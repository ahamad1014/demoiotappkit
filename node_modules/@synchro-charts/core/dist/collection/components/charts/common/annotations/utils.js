 ...
 * @param thresholds
 */
export const getNumberThresholds = (thresholds) => thresholds.filter(threshold => isNumeric(threshold.value));
/**
 * Returns only annotations defined for numbers
 * @param annotations
 */
export const getNumberAnnotations = (annotations) => {
 ...
 */
export const isHigherPriorityThresholds = (t1Array, t2) => {
    var _a;
 ...
 * @returns {Array.<Threshold>}
 */
export const highestPriorityThresholds = (thresholds) => {
 ...
 * Getters
 */
export const getColor = (annotation) => annotation.color;
export const getValueAndText = ({ annotation, resolution, viewport, }) => {
...
export const getText = (annotation) => {
    const labelText = annotation.label && annotation.label.show ? annotation.label.text : null;
    if (labelText) {
        return `${labelText}`;
    }
...
export const getLabelTextVisibility = (annotation) => {
    if (annotation.label && annotation.label.show) {
        return 'inline';
    }
    return 'none';
};
export const getValueTextVisibility = (annotation) => {
    if (annotation.showValue) {
        return 'inline';
    }
    return 'none';
};
export const getValueAndTextVisibility = (annotation) => {
...
export const getValueText = ({ annotation, resolution, viewport, formatText, }) => {
    var _a;
    const valueText = annotation.showValue
...
export const isThresholdBreached = (value, threshold) => {
 ...
 *  >= > <= <    >= > <= <
 */
export const sortThreshold = (thresholds) => [...thresholds].sort((a, b) => {
    if (a.value === b.value) {
 ...
 */
export const getBreachedThreshold = (value, thresholds) => {
...
export const isThreshold = isValid((maybeThreshold) => maybeThreshold.comparisonOperator != null);
export const getThresholds = (annotations) => annotations && annotations.y ? annotations.y.filter(isThreshold) : [];
