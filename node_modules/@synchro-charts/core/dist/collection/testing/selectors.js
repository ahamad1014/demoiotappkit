...
import { constructSearchQuery, SCREEN_SIZE } from './dynamicWidgetUtils/testCaseParameters';
...
export const waitForChart = (cy) => cy
    .get(CHART_VIZ_CONTAINER_SELECTOR)
    .should('be.visible')
    .wait(SMALL_WAIT)
    .click({ multiple: true });
export const waitForDataError = (cy) => cy
    .get(DATA_TYPE_ERROR_CONTAINER_SELECTOR)
    .should('be.visible')
    .wait(SMALL_WAIT)
    .click({ multiple: true });
export const route = (params) => `/tests/widget-test-route?${constructSearchQuery(params)}`;
export const infoNameTestSelector = (id) => `#info-${id}`;
export const DEFAULT_PARAMS = {
    dataStreams: [],
    dataStreamInfos: [],
};
export const visitDynamicWidget = (cy, params = {}) => {
    cy.viewport(SCREEN_SIZE.width, SCREEN_SIZE.height);
    cy.visit(route(Object.assign(Object.assign({}, DEFAULT_PARAMS), params)));
};
