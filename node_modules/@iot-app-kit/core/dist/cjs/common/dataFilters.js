...
exports.getDataBeforeDate = exports.getVisibleData = exports.pointBisector = void 0;
...
exports.pointBisector = (0, d3_array_1.bisector)((p) => p.x);
/**
 * Get Visible Data
 *
...
includeBoundaryPoints = true) => {
    ...
    const startIndex = Math.max(exports.pointBisector.left(data, start) - (includeBoundaryPoints ? 1 : 0), 0);
    const endIndex = Math.min(exports.pointBisector.right(data, end) - (includeBoundaryPoints ? 0 : 1), data.length - 1);
...
const getDataBeforeDate = (data, date) => {
    ...
    // Otherwise return all the data within the viewport, plus an additional single data point that falls outside of
    // the viewport in either direction.
    const endIndex = Math.min(exports.pointBisector.right(data, date) - 1, data.length - 1);
    return data.slice(0, endIndex + 1);
};
exports.getDataBeforeDate = getDataBeforeDate;
